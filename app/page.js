                ...
                {grpA.map((a, i) => {
                  ...
                  if (!isRel) return <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 16px", borderBottom: i < grpA.length - 1 ? "1px solid #F5F3EF" : "none", opacity: .5 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {dueDates[a.id] && <div style={{ fontFamily: F.b, fontSize: 10, color: "#CCC", marginTop: 1 }}>{dueDates[a.id]}</div>}
                    </div>
                  ...
                  return <div key={a.id} style={{ borderBottom: i < grpA.length - 1 ? "1px solid #F5F3EF" : "none" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 16px", cursor: "pointer" }}
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {dueDates[a.id] && <div style={{ fontFamily: F.b, fontSize: 10, color: isChecked ? "#CCC" : "#888", marginTop: 1 }}>{dueDates[a.id]}</div>}
                      </div>
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: "20px" }}>
        ...
        {currentPrep && <div>
          <div style={{ display: "flex", gap: 8, marginBottom: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <button onClick={() => { setEditDue(editDue === prepItem ? null : prepItem); setEditDueVal(dueDates[prepItem] || ''); }} style={{ padding: "2px 8px", border: "1px solid #E0DDD8", borderRadius: 4, fontFamily: F.b, fontSize: 9, color: dueDates[prepItem] ? "#856404" : "#CCC", cursor: "pointer", background: "#fff" }}>{dueDates[prepItem] ? "✎ Due: " + dueDates[prepItem] : "+ Due date"}</button>
          </div>
          {editDue === prepItem && <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
            <input value={editDueVal} onChange={e => setEditDueVal(e.target.value)} placeholder="e.g. Before class Mon 3/24" autoFocus style={{ flex: 1, padding: "5px 9px", border: "1px solid #E0DDD8", borderRadius: 5, fontFamily: F.b, fontSize: 11, outline: "none" }} onKeyDown={async e => { if (e.key === "Enter") { await upsertDueDate(ck, prepItem, editDueVal); setEditDue(null); refresh(); } }} />
            <button onClick={async () => { await upsertDueDate(ck, prepItem, editDueVal); setEditDue(null); refresh(); }} style={{ padding: "5px 10px", background: c.color, color: "#fff", border: "none", borderRadius: 5, fontFamily: F.b, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Save</button>
            <button onClick={async () => { await upsertDueDate(ck, prepItem, ''); setEditDue(null); refresh(); }} style={{ padding: "5px 8px", background: "#F5F4F0", color: "#999", border: "1px solid #E8E6E1", borderRadius: 5, fontFamily: F.b, fontSize: 10, cursor: "pointer" }}>Clear</button>
          </div>}
          <div style={{ display: "flex", gap: 6, marginBottom: 12 }}>
              ...
              {grp.ids.map((id, i) => { const a = c.assignments.find(x => x.id === id); if (!a) return null; const isR = rel.includes(id); const dd = dueDates[id]; const isEditingDue = editDue === id;
                return <div key={id} style={{ borderBottom: i < grp.ids.length - 1 ? "1px solid #F5F3EF" : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px" }}
                    ...
                    <button onClick={(e) => { e.stopPropagation(); setEditDue(isEditingDue ? null : id); setEditDueVal(dd || ''); }} style={{ padding: "2px 8px", border: "1px solid #E0DDD8", borderRadius: 4, fontFamily: F.b, fontSize: 9, color: dd ? "#856404" : "#CCC", cursor: "pointer", background: "#fff", flexShrink: 0 }}>{dd ? "✎ Due" : "+ Due date"}</button>
                  ...
                  {isEditingDue && <div style={{ padding: "4px 16px 10px 60px", display: "flex", gap: 6 }}>
                    ...
                    <button onClick={async () => { await upsertDueDate(ck, id, ''); setEditDue(null); refresh(); }} style={{ padding: "5px 8px", background: "#F5F4F0", color: "#999", border: "1px solid #E8E6E1", borderRadius: 5, fontFamily: F.b, fontSize: 10, cursor: "pointer" }}>Clear</button>
                  </div>}
                </div>;
              })}
            </div>
