              ...
              return <div key={g} style={{ marginBottom: 8, padding: "8px 12px", borderRadius: 8, border: isOn ? `2px solid ${m.c}` : "1px solid #F0EEEA", position: "relative" }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                  ...
                  {(t.pick || []).map((p, pi) => {
                    ...
                    return <span key={`pick-${pi}`} aria-label={`${completed.length} of ${p.need} ${p.label || 'items'} completed`} style={{ padding: "2px 6px", borderRadius: 5, fontFamily: F.b, fontSize: 11, background: completed.length >= p.need ? "#D4EDDA" : "#fff", border: `1px solid ${completed.length >= p.need ? "#B7DFBF" : "#E8E6E1"}`, color: completed.length >= p.need ? "#2D6A4F" : "#555", fontStyle: "italic" }}>{completed.length >= p.need ? "✓ " : ""}{completed.length}/{p.need} {p.label || "items"}</span>;
                  })}
