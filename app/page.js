...
export const COURSES = {
  "ECEL 3820": {
    ...
    tracks: {
      A: { req: ["3act","lesson","frac-int","frac-act","thinking","portfolio"], pick: [{ from: ["vtm1","vtm2","vtm3"], need: 3 }] },
      B: { req: ["3act","lesson","frac-int","frac-act","portfolio"], pick: [{ from: ["vtm1","vtm2","vtm3"], need: 2 }] },
      C: { req: ["3act","lesson","frac-int","portfolio"], pick: [{ from: ["vtm1","vtm2","vtm3"], need: 1 }] },
  ...
  "ECEL 4850": {
    ...
    tracks: {
      A: { req: ["les1","les2","ct1","ct2","nt-plan","nt-impl","pwa-chart","pwa-followup"], pick: [{ from: ["pwa-a1","pwa-a2","pwa-a3"], need: 3 }, { from: ["fp-w1","fp-w2","fp-w3"], need: 3 }] },
      B: { req: ["les1","les2","ct1","ct2","nt-plan","pwa-chart"], pick: [{ from: ["pwa-a1","pwa-a2","pwa-a3"], need: 2 }, { from: ["fp-w1","fp-w2","fp-w3"], need: 2 }] },
      C: { req: ["pwa-chart"], pick: [{ from: ["les1","les2"], need: 1 }, { from: ["ct1","ct2"], need: 1 }, { from: ["pwa-a1","pwa-a2","pwa-a3"], need: 1 }, { from: ["fp-w1","fp-w2","fp-w3"], need: 1 }] },
      D: { req: [], pick: [{ from: ["les1","les2"], need: 1 }, { from: ["ct1","ct2"], need: 1 }, { from: ["fp-w1","fp-w2","fp-w3"], need: 1 }] },
    },
  },
  "ECEL 3468": {
...
/* ================================================================
   SHARED HELPER — checks if a track's requirements are met
   Handles: req (fixed), pick (any N from pool), isOr/alt (D track)
   ================================================================ */
function trackMet(track, relIds, done) {
  // Fixed requirements
  const rv = track.req.filter(id => relIds.includes(id));
  if (!rv.every(id => done.has(id))) return false;
  // Pick requirements (any N from pool)
  for (const p of (track.pick || [])) {
    const completed = p.from.filter(id => relIds.includes(id) && done.has(id));
    if (completed.length < p.need) return false;
  }
  return true;
}

function trackHasRelevantItems(track, relIds) {
  if (track.req.some(id => relIds.includes(id))) return true;
  if ((track.pick || []).some(p => p.from.some(id => relIds.includes(id)))) return true;
  return false;
}

/* Collect ALL item IDs that a track cares about (req + pick pools) */
export function trackAllIds(track) {
  const ids = [...track.req];
  for (const p of (track.pick || [])) { ids.push(...p.from); }
  if (track.alt) ids.push(...track.alt);
  return ids;
}

/* For a given track, figure out which items are still needed.
   Returns { blockers: [...ids], pickBlockers: [{ from, need, have, missing }] } */
export function getTrackBlockers(track, relIds, done) {
  const blockers = track.req.filter(id => relIds.includes(id) && !done.has(id));
  const pickBlockers = (track.pick || []).map(p => {
    const completed = p.from.filter(id => relIds.includes(id) && done.has(id));
    const still = p.need - completed.length;
    if (still <= 0) return null;
    const missing = p.from.filter(id => relIds.includes(id) && !done.has(id));
    const notReleased = p.from.filter(id => !relIds.includes(id));
    return { from: p.from, need: p.need, have: completed.length, still, missing, notReleased };
  }).filter(Boolean);
  return { blockers, pickBlockers };
}
...
export function calcGrade(checked, relIds, ck) {
  ...
  for (const g of ["A", "B", "C", "D"]) {
    ...
    if (g === "D" && t.isOr) {
      ...
      const pickOk = (t.pick || []).every(p => p.from.filter(id => relIds.includes(id) && done.has(id)).length >= p.need);
      if (((t.req.length === 0 || (t.req.some(id => relIds.includes(id)) && mOk)) || (t.alt && t.alt.some(id => relIds.includes(id)) && aOk)) && pickOk) return g;
    } else {
      if (trackMet(t, relIds, done)) return g;
    }
  }
  return "F";
}
...
export function getBlockers(checked, relIds, ck) {
  ...
  if (grade === "early") return { target: null, blockers: [], pickBlockers: [], msg: "Check off your first assignment to see your grade track!" };
  const order = ["A", "B", "C", "D", "F"];
  const idx = order.indexOf(grade);
  const target = idx > 0 ? order[idx - 1] : null;
  if (!target) return { target: null, blockers: [], pickBlockers: [], msg: grade === "A" ? "You're on the highest track!" : "" };
  const t = COURSES[ck].tracks[target];
  if (!t) return { target, blockers: [], pickBlockers: [], msg: "" };
  const done = new Set(Object.keys(checked).filter(k => checked[k] === true || checked[k] === "mastery"));
  const { blockers, pickBlockers } = getTrackBlockers(t, relIds, done);
  const totalRemaining = blockers.length + pickBlockers.reduce((sum, pb) => sum + pb.still, 0);
  return { target, blockers, pickBlockers, msg: totalRemaining === 0 ? `Remaining ${target}-track items haven't been assigned yet.` : `${totalRemaining} more to reach ${target} track` };
}
