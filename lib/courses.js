...
export let COURSES = { ...COURSES_FALLBACK };

// Hydrate COURSES from a DB row
export function hydrateFromDB(rows) {
  ...
  // Merge: DB overrides fallback, but keep any fallback courses not in DB
  COURSES = { ...COURSES_FALLBACK, ...result };
  return COURSES;
}
