...
// CURRENT TERM
// ----------------------------------------------------------------------------
// Every row written to a term-scoped table is stamped with this value.
// Change it once per semester (Build 1b adds a Settings-tab dropdown so this
// stops being a code edit).
//
// Format: two-letter season + two-digit year.
//   SP = Spring (Jan–May), SU = Summer (May–Jul), FA = Fall (Aug–Dec)
//
// Build 1a: writes stamp this value; reads still ignore term entirely.
// Do NOT roll this forward until Build 1b is deployed — until reads filter on
// term, changing it only mislabels new rows.
// ============================================================================
export const CURRENT_TERM = "SU26";

// NOTE: tokenCutoff / tokenCutoffDate below are DEFAULTS ONLY as of Build 1d.
// The live value comes from term_settings (course_key, term) in Supabase and is
// edited from Settings > Token Cutoffs. These constants are the fallback used
// when no row exists for the current term.
...
export const COURSES = {
  "ECEL 3820": {
    ...
    sections: {
      LS: { name: "Lee's Summit", code: "MATH3820LS" },
      WB: { name: "Warrensburg", code: "MATH3820WB" },
    },
    ...
    tracks: {
      ...
      C: { req: ["3act","lesson","frac-int","portfolio"], pick: [{ from: ["vtm1","vtm2","vtm3"], need: 1, label: "VTM" }] },
  ...
  "ECEL 4850": {
    ...
    tokenCutoff: "April 26, 2026", tokenCutoffDate: new Date("2026-04-26T23:59:59"),
    groups: [
      { name: null, ids: ["les1","les2","ct1","ct2","lcd"] },
      ...
      { name: "Final Portfolio", ids: ["fp-w1","fp-w2","fp-w3","fp-w4"] },
    ],
    assignments: [
      { id: "les1", name: "LES Lesson Plan 1", eval: "mastery", teachingSchedule: true },
      { id: "les2", name: "LES Lesson Plan 2", eval: "mastery", teachingSchedule: true },
      { id: "ct1", name: "CT Feedback & Reflection 1", eval: "completion" },
      { id: "ct2", name: "CT Feedback & Reflection 2", eval: "completion" },
      { id: "lcd", name: "Levels of Cognitive Demand Online Assignment", eval: "completion", sections: ["WB"] },
      { id: "nt-plan", name: "Number Talk Planning", eval: "mastery", teachingSchedule: true },
      ...
      { id: "fp-w1", name: "SPLAT", eval: "completion" },
      { id: "fp-w2", name: "Which One Doesn't Belong", eval: "completion" },
      { id: "fp-w3", name: "Esti-Mystery", eval: "completion" },
      { id: "fp-w4", name: "Cube Conversations", eval: "completion" },
    ],
    ...
    tracks: {
      A: { req: ["les1","les2","ct1","ct2","nt-plan","nt-impl","pwa-chart","pwa-a1","pwa-a2","pwa-a3","pwa-followup"], pick: [{ from: ["fp-w1","fp-w2","fp-w3","fp-w4"], need: 3, label: "warm ups" }] },
      B: { req: ["les1","les2","ct1","ct2","nt-plan","pwa-chart"], pick: [{ from: ["pwa-a1","pwa-a2","pwa-a3"], need: 2, label: "analyses" }, { from: ["fp-w1","fp-w2","fp-w3","fp-w4"], need: 2, label: "warm ups" }] },
      C: { req: ["pwa-chart"], pick: [{ from: ["les1","les2"], need: 1, label: "lesson plan" }, { from: ["ct1","ct2"], need: 1, label: "reflection" }, { from: ["pwa-a1","pwa-a2","pwa-a3"], need: 1, label: "analysis" }, { from: ["fp-w1","fp-w2","fp-w3","fp-w4"], need: 1, label: "warm up" }] },
      D: { req: [], pick: [{ from: ["les1","les2"], need: 1, label: "lesson plan" }, { from: ["ct1","ct2"], need: 1, label: "reflection" }, { from: ["fp-w1","fp-w2","fp-w3","fp-w4"], need: 1, label: "warm up" }] },
    },
  },
  "ECEL 3468": {
    ...
    bonus: [],
    tracks: {
      // Bundles: any N of 5. Journals: any N of 4. AI scenarios: any N of 2.
      // sp (special project) is a single required mastery item at A only.
      A: { req: ["sp"], pick: [ { from: ["b1","b2","b3","b4","b5"], need: 5 }, { from: ["j1","j2","j3","j4"], need: 4 }, { from: ["ai1","ai2"], need: 2 } ] },
      B: { pick: [ { from: ["b1","b2","b3","b4","b5"], need: 5 }, { from: ["j1","j2","j3","j4"], need: 4 }, { from: ["ai1","ai2"], need: 1 } ] },
      C: { pick: [ { from: ["b1","b2","b3","b4","b5"], need: 4 }, { from: ["j1","j2","j3","j4"], need: 3 } ] },
      D: { pick: [ { from: ["b1","b2","b3","b4","b5"], need: 3 }, { from: ["j1","j2","j3","j4"], need: 2 } ] },
    },
  },
};
...
function checkTrackRequirements(track, availableIds, doneIds) {
  ...
  const pickMet = (track.pick || []).every(p => {
    ...
    // Cap the requirement at how many pool items have actually been assigned.
    // Early in the term a pool may have fewer items available than `need`;
    // a student who has done all the assigned ones should not be held back by
    // items that don't exist yet. min() only ever lowers the bar when fewer
    // than `need` are assigned — once the full pool is assigned it equals `need`.
    return completed.length >= Math.min(p.need, available.length);
  });
...
/* ================================================================
   ...
   The student's grade is entirely a product of their own actions
   and course due dates. Instructor actions control access
   (locking/unlocking the checkbox) but do not automatically
   improve the student's grade.

   Mastery items:
   - Blank/NS: invisible to calc (no evaluation yet, or not submitted)
   - R: enters calc as not-done — grade drops (accurate: needs revision)
   - M + not checked off: invisible — no drop, checkbox unlocks
   - M + checked off: enters calc as done — grade improves

   Completion items:
   - Invisible until due date passes OR student checks off early
   - Instructor markings (M, NS) have no effect on student calc
   ================================================================ */
export function calcStudentGrade(studentChecks, instrStatuses, relIds, ck, dueDates = {}) {
  ...
  for (const id of relIds) {
    ...
    const instrStatus = instrStatuses[id]; // "mastery", "revision", "not_submitted", or undefined
    ...
    if (a.eval === "completion") {
      ...
      // Mastery items — student grade rules:
      // - Blank: invisible. Instructor hasn't evaluated yet — counts neither for nor against.
      // - NS: enters calc as not-done. A confirmed miss (instructor checked, nothing submitted
      //   by the deadline). Grade drops; item stays LOCKED. Student recovers by submitting a
      //   token, after which the instructor marks R or M.
      // - R: enters calc as not-done. Grade drops — accurate signal that revision is needed.
      // - M + not checked off: enters calc as NOT-DONE (grade-relevant but unconfirmed).
      //   The instructor's M does not move the student's grade — the item holds its slot in
      //   the calc as "not done by the student yet," so the grade stays exactly where it was
      //   (blank/R/NS level) until SHE checks off. This prevents the instructor's mark from
      //   shrinking a pick-pool denominator and silently raising the grade. (Option A,
      //   June 28 2026 — see Lumos-Grade-Calculation-Spec.md.)
      // - M + checked off: enters calc as done. Grade improves. This is the ownership moment.
      // Instructor actions never automatically improve the student's grade — only their checkoff does.
      if (instrStatus === "revision" || instrStatus === "not_submitted") {
        gradeRelevant.add(id);
        // R or NS is in the calc but not confirmed — actively counts against grade
      } else if (instrStatus === "mastery") {
        gradeRelevant.add(id);
        // M is in the calc; confirmed ONLY when the student has checked off.
        if (studentChecked) confirmed.add(id);
      }
    }
  }
...
export function getStudentBlockers(studentChecks, instrStatuses, relIds, ck, dueDates = {}) {
  ...
  for (const id of relIds) {
    ...
    if (a.eval === "completion") {
      ...
      // Mastery items: R enters calc as not-done; M + checked off enters as done
      // Blank/NS stay invisible
      const instrStatus = instrStatuses[id];
      if (instrStatus === "revision") {
        gradeRelevant.add(id);
      } else if (instrStatus === "mastery" && studentChecked) {
        gradeRelevant.add(id);
        confirmed.add(id);
      }
    }
...
export function getBlockers(checked, relIds, ck, instrStatuses = null, dueDates = null) {
  // If instrStatuses provided, use dual-gate grade calc (student view)
  const isDualGate = instrStatuses !== null;
  const grade = isDualGate ? calcStudentGrade(checked, instrStatuses, relIds, ck, dueDates || {}) : calcGrade(checked, relIds, ck);
  if (grade === "early") return { target: null, blockers: [], msg: "Check off your first assignment to see your grade track!" };
  const c = COURSES[ck];

  // Build the "done" set — in dual-gate mode, only confirmed items count
  const done = new Set();
  if (isDualGate) {
    for (const id of relIds) {
      const a = c.assignments.find(x => x.id === id);
      if (!a) continue;
      const studentChecked = checked[id] === true || checked[id] === "mastery";
      if (a.eval === "completion" && studentChecked) {
        done.add(id);
      } else if (a.eval === "mastery" && (instrStatuses[id] === "mastery") && studentChecked) {
        done.add(id);
      }
    }
  } else {
    Object.keys(checked).filter(k => checked[k] === true || checked[k] === "mastery").forEach(k => done.add(k));
  }

  // In dual-gate mode, only show blockers for grade-relevant items
  // (items whose due date has passed or instructor has evaluated)
  const isRelevant = (id) => {
    if (!isDualGate) return relIds.includes(id);
    const a = c.assignments.find(x => x.id === id);
    if (!a) return false;
    if (a.eval === "completion") {
      const hasDueDate = !!(dueDates && dueDates[id]?.date);
      const dueDatePassed = hasDueDate && new Date(dueDates[id].date + 'T23:59:59') < new Date();
      return dueDatePassed || done.has(id);
    } else {
      // Mastery items: R and NS are grade-relevant (not-done); M is grade-relevant
      // (done only when the student has checked off — Option A, June 28 2026).
      // Blank stays invisible (instructor hasn't evaluated yet).
      return instrStatuses[id] === "revision" || instrStatuses[id] === "not_submitted" || instrStatuses[id] === "mastery";
    }
  };

  // Target = the HIGHEST track the student could reach by clearing only their
  // currently grade-relevant outstanding items. We do NOT use grade+1: clearing
  // one shared blocker (e.g. an R on an item required by multiple tracks) can
  // satisfy several tracks at once. A track is reachable if, treating every
  // currently-relevant item as done, its requirements are met — this reuses the
  // same relevance gate as the grade calc, so tracks that require not-yet-due
  // work are skipped as "not yet applicable" and never become the target.
  // Target selection (Option 3): show the next concrete rung, but if a higher
  // track is reachable by clearing the SAME items, jump to that higher track.
  //
  // Step 1: find the lowest track the student has NOT already satisfied with
  //   work actually done (walking D -> A). That's the honest "next rung."
  // Step 2: from that rung upward, climb to the highest track whose outstanding
  //   items are a subset of the next rung's outstanding items. This means a
  //   single shared blocker (e.g. one journal short, which blocks D, C, B, and A
  //   equally) correctly targets A rather than D — while a student who is many
  //   items short of A still only sees the small next-rung list.
  //
  // "Outstanding for a track" = items needed to satisfy that track given what's
  // currently relevant and done, computed via collectTrackBlockers below.
  const collectTrackBlockers = (tk) => {
    const bl = [];
    (tk.req || []).filter(id => isRelevant(id) && !done.has(id)).forEach(id => bl.push(id));
    (tk.pick || []).forEach(p => {
      const available = p.from.filter(id => isRelevant(id));
      const completed = available.filter(id => done.has(id));
      const remaining = p.need - completed.length;
      if (remaining > 0) available.filter(id => !done.has(id)).slice(0, remaining).forEach(id => { if (!bl.includes(id)) bl.push(id); });
    });
    (tk.pickGroup || []).forEach(pg => {
      let groupsCompleted = 0;
      for (const group of pg.from) {
        const ga = group.filter(id => isRelevant(id));
        if (ga.length > 0 && ga.every(id => done.has(id))) groupsCompleted++;
      }
      if (pg.need - groupsCompleted > 0) {
        for (const group of pg.from) {
          const ga = group.filter(id => isRelevant(id));
          if (!ga.every(id => done.has(id))) { ga.filter(id => !done.has(id)).forEach(id => { if (!bl.includes(id)) bl.push(id); }); break; }
        }
      }
    });
    return bl;
  ...
  const ladder = ["D", "C", "B", "A"]; // lowest to highest
  // Step 1: lowest track not yet satisfied by work actually done.
  let nextRung = null;
  for (const g of ladder) {
    const tk = COURSES[ck].tracks[g];
    if (!tk || !trackHasRelevantItems(tk, relIds.filter(id => isRelevant(id)))) continue;
    if (!checkTrackRequirements(tk, relIds.filter(id => isRelevant(id)), done)) { nextRung = g; break; }
  }
  // If every relevant track is already satisfied, student is at the top reachable now.
  if (!nextRung) return { target: null, blockers: [], msg: grade === "A" ? "You're on the highest track!" : "" };

  const rungBlockers = collectTrackBlockers(COURSES[ck].tracks[nextRung]);
  const rungSet = new Set(rungBlockers);
  // Step 2: climb to the highest track whose outstanding items are a subset of
  // the next rung's outstanding items (i.e. no EXTRA work beyond this rung).
  let target = nextRung;
  for (let i = ladder.indexOf(nextRung) + 1; i < ladder.length; i++) {
    const g = ladder[i];
    const tk = COURSES[ck].tracks[g];
    if (!tk || !trackHasRelevantItems(tk, relIds.filter(id => isRelevant(id)))) continue;
    const gb = collectTrackBlockers(tk);
    if (gb.every(id => rungSet.has(id))) target = g; else break;
  }
  const t = COURSES[ck].tracks[target];
  if (!t) return { target, blockers: [], msg: "" };
  
  const blockers = (t.req || []).filter(id => isRelevant(id) && !done.has(id));
  ...
  (t.pickGroup || []).forEach(pg => {
    let groupsCompleted = 0;
    for (const group of pg.from) {
      const groupAvailable = group.filter(id => isRelevant(id));
