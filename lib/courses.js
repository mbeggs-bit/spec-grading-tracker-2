...
/* ================================================================
   ...
   and course due dates. Instructor actions (M, R, NS) control
   access (locking/unlocking the checkbox) but do NOT directly
   move the student's displayed grade.

   An assignment enters the student's grade calc only when:
   - Completion items: due date has passed OR student checked off early
   - Mastery items: instructor marked M AND student has checked off

   Mastery items with no evaluation, R, or NS stay invisible to
   the student's grade calc regardless of due date. Only when the
   student checks off after an M does the item count as done and
   improve their grade. Instructor actions never drop the student's
   grade — only missed due dates (completion) or the absence of a
   student checkoff can do that.
   ================================================================ */
export function calcStudentGrade(studentChecks, instrStatuses, relIds, ck, dueDates = {}) {
  ...
  for (const id of relIds) {
    ...
    if (a.eval === "completion") {
      ...
      // Mastery items: only enter calc when instructor marked M AND student checked off.
      // R, NS, and no-status all stay invisible — instructor actions never move the
      // student's grade. The checkbox lock/unlock is the only bridge between sides.
...
export function getStudentBlockers(studentChecks, instrStatuses, relIds, ck, dueDates = {}) {
  ...
  for (const id of relIds) {
    ...
    if (a.eval === "completion") {
      ...
      // Mastery items: only grade-relevant when M + student checked off
      // R, NS, no-status stay invisible in student view
...
export function getBlockers(checked, relIds, ck, instrStatuses = null, dueDates = null) {
  ...
  const isRelevant = (id) => {
    ...
    if (a.eval === "completion") {
      ...
      // Mastery items only appear as blockers when the student has confirmed them (M + checked off)
      // R, NS, and no-status stay invisible — they don't appear in the student's grade calc
      const studentChecked = checked[id] === true || checked[id] === "mastery";
      return instrStatuses[id] === "mastery" && studentChecked;
    }
  };
