...
export const COURSES = {
  ...
  "ECEL 4850": {
    ...
    tracks: {
      A: { req: ["les1","les2","ct1","ct2","nt-plan","nt-impl","pwa-chart","pwa-a1","pwa-a2","pwa-a3","pwa-followup"], pick: [{ from: ["fp-w1","fp-w2","fp-w3"], need: 3 }] },
      B: { req: ["les1","les2","ct1","ct2","nt-plan","pwa-chart"], pick: [{ from: ["pwa-a1","pwa-a2","pwa-a3"], need: 2 }, { from: ["fp-w1","fp-w2","fp-w3"], need: 2 }] },
      C: { req: ["pwa-chart"], pickGroup: [{ from: [["les1","ct1"], ["les2","ct2"]], need: 1 }], pick: [{ from: ["pwa-a1","pwa-a2","pwa-a3"], need: 1 }, { from: ["fp-w1","fp-w2","fp-w3"], need: 1 }] },
      D: { req: [], pickGroup: [{ from: [["les1","ct1"], ["les2","ct2"]], need: 1 }], pick: [{ from: ["fp-w1","fp-w2","fp-w3"], need: 1 }] },
    },
  },
  "ECEL 3468": {
...
function checkTrackRequirements(track, availableIds, doneIds) {
  ...
  const pickGroupMet = (track.pickGroup || []).every(pg => {
    ...
    for (const group of pg.from) {
      ...
      // Only count a group if at least one item is available
...
/* ================================================================
   ...
   - Completion items: student has checked it off (no instructor gate)
   ...
   ================================================================ */
export function calcStudentGrade(studentChecks, instrStatuses, relIds, ck) {
  const c = COURSES[ck];
  ...
  for (const id of relIds) {
    ...
    if (a.eval === "completion") {
      // Completion items: always in the calc once released, done when checked
...
/* ================================================================
   ...
   ================================================================ */
export function getStudentBlockers(studentChecks, instrStatuses, relIds, ck) {
  const grade = calcStudentGrade(studentChecks, instrStatuses, relIds, ck);
  ...
  // A blocker is a released assignment in the target track that isn't fully confirmed
  const confirmed = new Set();
  for (const id of relIds) {
    const a = c.assignments.find(x => x.id === id);
    if (!a) continue;
    const studentChecked = studentChecks[id] === true || studentChecks[id] === "mastery";
    if (a.eval === "completion" && studentChecked) confirmed.add(id);
    if (a.eval !== "completion" && instrStatuses[id] === "mastery" && studentChecked) confirmed.add(id);
  }
  
  const blockers = t.req.filter(id => relIds.includes(id) && !confirmed.has(id));
  ...
  (t.pickGroup || []).forEach(pg => {
    ...
    if (remaining > 0) {
      // Show incomplete group items as blockers
      for (const group of pg.from) {
        const groupAvailable = group.filter(id => relIds.includes(id));
        ...
        if (!groupDone) {
          ...
          break; // Only show one incomplete group's items
        }
      }
    }
  });
