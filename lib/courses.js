...
// Fallback course definitions — used if course_config table is empty or unavailable
export const COURSES_FALLBACK = {
  "ECEL 3820": {
...
// Keep COURSES as a mutable reference that gets hydrated from DB
export const COURSES = { ...COURSES_FALLBACK };

// Hydrate COURSES from a DB row — mutates the existing object so all importers see changes
export function hydrateFromDB(rows) {
  const result = {};
  (rows || []).forEach(r => {
    result[r.course_key] = {
      title: r.title,
      ...
      tokenCutoff: r.token_cutoff,
      tokenCutoffDate: r.token_cutoff_date ? new Date(r.token_cutoff_date) : null,
      assignments: r.assignments || [],
      groups: r.groups || [],
      classPrep: r.class_prep || [],
      tokenGroups: r.token_groups || {},
      tracks: r.tracks || {},
      sections: r.sections || null,
      bonus: r.bonus || [],
    };
  });
  // Clear existing keys and merge: DB overrides fallback
  Object.keys(COURSES).forEach(k => delete COURSES[k]);
  Object.assign(COURSES, COURSES_FALLBACK, result);
  return COURSES;
}

// Convert app-format course config back to DB row format for saving
export function courseToDBRow(courseKey, c) {
  return {
    course_key: courseKey,
    title: c.title,
    short: c.short,
    color: c.color,
    color_light: c.colorLight,
    token_cutoff: c.tokenCutoff,
    token_cutoff_date: c.tokenCutoffDate ? c.tokenCutoffDate.toISOString() : null,
    assignments: c.assignments,
    groups: c.groups,
    class_prep: c.classPrep || [],
    token_groups: c.tokenGroups || {},
    tracks: c.tracks,
...
export function calcGrade(checked, relIds, ck, courseOverride) {
  const c = courseOverride || COURSES[ck];
...
export function getBlockers(checked, relIds, ck, courseOverride) {
  const grade = calcGrade(checked, relIds, ck, courseOverride);
...
export function pastCutoff(ck, courseOverride) {
  const c = courseOverride || COURSES[ck];
  if (!c || !c.tokenCutoffDate) return false;
  return new Date() > c.tokenCutoffDate;
}

export function getTokenCutoff(ck, courseOverride) {
  const c = courseOverride || COURSES[ck];
  return c?.tokenCutoff || "";
}

export function getTokenTarget(aId, ck, courseOverride) {
  const c = courseOverride || COURSES[ck];
  const a = c.assignments.find(x => x.id === aId);
  if (a?.tokenGroup) {
    const g = c.tokenGroups[a.tokenGroup];
    return { id: a.tokenGroup, name: g?.name || a.tokenGroup, isGroup: true };
  }
  return { id: aId, name: a?.name || aId, isGroup: false };
}

export function getCourseSections(ck, courseOverride) {
  const c = courseOverride || COURSES[ck];
