...
/* ================================================================
   ...
   - Mastery items: ONLY when instructor has marked mastery AND student
     has checked it off. If either side hasn't acted, the assignment is
     completely invisible to the grade math — no penalty, no boost.
   ================================================================ */
export function calcStudentGrade(studentChecks, instrStatuses, relIds, ck) {
  ...
  for (const id of relIds) {
    ...
    if (a.eval === "completion") {
      ...
      // Mastery items: only enter grade calc when both sides have confirmed mastery
      if (instrStatus === "mastery" && studentChecked) {
        gradeRelevant.add(id);
        confirmed.add(id);
      }
      // If instructor marked R or hasn't acted, or student hasn't checked off:
      // assignment is invisible to grade math
    }
  }
  ...
  for (const g of ["A", "B", "C", "D"]) {
    ...
    if (g === "D" && t.isOr) {
      ...
      if (rv.every(id => confirmed.has(id))) return g;
    }
  }
  return "F";
}
