...
export function calcStudentGrade(studentChecks, instrStatuses, relIds, ck, dueDates = {}) {
  ...
  for (const id of relIds) {
    ...
    if (a.eval === "completion") {
      ...
      // - M + not checked off: invisible. No drop; checkbox unlocks for student to claim credit.
      // - M + checked off: enters calc as done. Grade improves.
...
export function getBlockers(checked, relIds, ck, instrStatuses = null, dueDates = null) {
  ...
  const isRelevant = (id) => {
    ...
    if (a.eval === "completion") {
      ...
      // Mastery items: R and NS are grade-relevant (not-done); M + checked off is grade-relevant (done).
      // Blank stays invisible (instructor hasn't evaluated yet). NS is a confirmed miss and counts.
      const studentChecked = checked[id] === true || checked[id] === "mastery";
      return instrStatuses[id] === "revision" || instrStatuses[id] === "not_submitted" || (instrStatuses[id] === "mastery" && studentChecked);
    }
  };
