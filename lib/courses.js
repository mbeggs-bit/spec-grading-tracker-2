...
/* ================================================================
   GRADE CALCULATION — INSTRUCTOR VIEW (unchanged)
...
export function calcGrade(checked, relIds, ck) {
  ...
  for (const g of ["A", "B", "C", "D"]) {
    ...
    if (rv.length === 0 && !(t.pick || []).some(p => p.from.some(id => relIds.includes(id)))) continue;
    if (g === "D" && t.isOr) {
      ...
      const reqMet = rv.every(id => done.has(id));
...
export function calcStudentGrade(studentChecks, instrStatuses, relIds, ck) {
  ...
  for (const g of ["A", "B", "C", "D"]) {
    const t = c.tracks[g];
    const rv = t.req.filter(id => gradeRelIds.includes(id));
    if (rv.length === 0 && !(t.pick || []).some(p => p.from.some(id => gradeRelIds.includes(id)))) continue;
    if (g === "D" && t.isOr) {
      ...
      const reqMet = rv.every(id => confirmed.has(id));
      const pickMet = (t.pick || []).every(p => {
...
/* ================================================================
   BLOCKERS — INSTRUCTOR VIEW (unchanged)
   ================================================================ */
