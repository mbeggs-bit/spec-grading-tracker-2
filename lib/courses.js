...
export function calcStudentGrade(studentChecks, instrStatuses, relIds, ck, dueDates = {}) {
  ...
  for (const id of relIds) {
    ...
    if (a.eval === "completion") {
      ...
      // - Blank/NS: invisible. Instructor hasn't evaluated or flagged no submission.
      ...
      if (instrStatus === "revision") {
        gradeRelevant.add(id);
        // R is in the calc but not confirmed — actively counts against grade
...
export function getBlockers(checked, relIds, ck, instrStatuses = null, dueDates = null) {
  ...
  const isRelevant = (id) => {
    ...
    if (a.eval === "completion") {
      ...
      // Mastery items: R is grade-relevant (not-done); M + checked off is grade-relevant (done)
      // Blank and NS stay invisible in student view
      const studentChecked = checked[id] === true || checked[id] === "mastery";
      return instrStatuses[id] === "revision" || (instrStatuses[id] === "mastery" && studentChecked);
    }
  };
